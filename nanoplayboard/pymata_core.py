"""
NanoPlayBoard PyMataCore class.
...
class PymataCore:
    ...
    def start(self):
        ...
        if self.ip_address:
            self.socket = PymataSocket(
                self.ip_address, self.ip_port, self.loop)
    ...
    async def extended_analog(self, pin, data):
        ...
        :returns: No return value
        """
        analog_data = [pin, data & 0x7f,
                       (data >> 7) & 0x7f, (data >> 14) & 0x7f]
    ...
    async def pixy_set_servos(self, s0, s1):
        ...
        :returns: No return value.
        """
        data = [PrivateConstants.PIXY_SET_SERVOS, s0 & 0x7f,
                (s0 >> 7) & 0x7f, s1 & 0x7f, (s1 >> 7) & 0x7f]
        await self._send_sysex(PrivateConstants.PIXY_CONFIG, data)
    ...
    async def pixy_set_brightness(self, brightness):
        ...
        :returns: No return value.
        """
        data = [PrivateConstants.PIXY_SET_BRIGHTNESS,
                brightness & 0x7f, (brightness >> 7) & 0x7f]
        await self._send_sysex(PrivateConstants.PIXY_CONFIG, data)
    ...
    async def _command_dispatcher(self):
        ...
        while True:
            try:
                ...
                # A error occurred while transmitting the Firmata message,
                # message arrived invalid.
                if self.log_output:
    ...
    async def _capability_response(self, data):
        ...
        self.query_reply_data[
            PrivateConstants.CAPABILITY_RESPONSE] = data[1:-1]
    ...
    async def _pixy_data(self, data):
        ...
        # Prepare the new blocks list and then used it to overwrite the
        # pixy_blocks.
        blocks = []
        # noinspection PyDictCreation
        ...
        if self.digital_pins[PrivateConstants.PIN_PIXY_MOSI].cb:
            if self.digital_pins[PrivateConstants.PIN_PIXY_MOSI].cb_type:
                ...
                loop = self.loop
                loop.call_soon(self.digital_pins[
                               PrivateConstants.PIN_PIXY_MOSI].cb, blocks)
